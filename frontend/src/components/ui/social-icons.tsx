...
export function SocialIcons({ socials }: SocialIconsProps) {
  ...
  const socialLinks: Social[] = [
    ...
    ...(socials?.email ? [{
      ...
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="size-[18px]" strokeWidth="2">
  ...
  return (
    <div className="relative flex items-center gap-0.5 px-1.5 py-1.5 rounded-2xl bg-background/80 border border-border/40">
      ...
      {socialLinks.map((social, index) => (
        ...
        >
          ...
          <span
            className={`absolute bottom-1.5 left-1/2 -translate-x-1/2 h-[2px] rounded-full bg-primary transition-all duration-300 ease-out ${
